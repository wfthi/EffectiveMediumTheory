c      subroutine br_cde(nwave,ninc,e_mat,e_inc,vol_frac,
c     *                           tol,niter,e_br,iter,error,chisq) 
c
...
c 
      subroutine br_cde(max_nwave,max_ninc,
...
c     input
      ...
      double precision vol_frac(1:max_ninc),tol
      complex e_mat(1:max_nwave),e_inc(1:max_ninc,1:max_nwave)
c     output
      integer iter
      complex e_br(1:max_nwave)
c     local variables
      integer inc,wave
      double precision chisq,chisq2,mchisq,vol_tot,min_chisq
      complex ratio(1:max_nwave),alpha_CDE(1:max_nwave),sum_inc,
     * e_ave(1:max_nwave),e_br2(1:max_nwave)
...
c --- compute the polarizabilities with as first guess use for e_br formula for spherical inclusions
      do wave = 1,nwave
         sum_inc = (0.0,0.0)
         do inc = 1,ninc
            e_br(wave)=e_mat(wave)*(1.+(3.*vol_frac(inc)*
     *  (e_inc(inc,wave)-e_mat(wave))/(e_inc(inc,wave)+2.*e_mat(wave)))/
     *  (1.-vol_frac(inc)*(e_inc(inc,wave)-e_mat(wave))/(e_inc(inc,wave)
     *   +2.*e_mat(wave))))
            ratio(wave)=e_inc(inc,wave)/e_br(wave)
