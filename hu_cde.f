c      subroutine hu_cde(nwave,ninc,e_mat,e_inc,vol_frac,
c     *                           tol,niter,e_hu,iter,error,chisq) 
c
c     Purpose: Compute the effective permeability using the
c              Hunderi formulation for continuous distribution
...
c 
      subroutine hu_cde(max_nwave,max_ninc,
     *                           nwave,ninc,e_mat,e_inc,vol_frac,
     *                           tol,niter,e_hu,iter,error,chisq) 
      implicit none
c --- parameters
...
c     local variables
     ...
     *        e_ave(1:max_nwave),e_hu2(1:max_nwave)
...
c --- compute the polarizabilities with as first guess use for e_hu formula for spherical inclusions

      do wave = 1,nwave
         sum_inc = (0.0,0.0)
         do inc = 1,ninc
            e_hu(wave)=e_mat(wave)*(1.+(3.*vol_frac(inc)*
     *  (e_inc(inc,wave)-e_mat(wave))/(e_inc(inc,wave)+2.*e_mat(wave)))/
     *  (1.-vol_frac(inc)*(e_inc(inc,wave)-e_mat(wave))/(e_inc(inc,wave)
     *   +2.*e_mat(wave))))
            ratio(wave)=e_inc(inc,wave)/e_hu(wave)
