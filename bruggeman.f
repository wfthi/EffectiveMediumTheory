c      subroutine bruggeman(nwave,ninc,e_mat,e_inc,vol_frac,L1,L2,
c     *                           tol,niter,e_mg,iter,error,chisq) 
c
c     Purpose: Compute the effective permeability using the generalized
c              Bruggeman formulation for abitrary shape and number
...
c
c     output
c     e_br(1:max_nwave)   complex effective permeability of the effective medium
c                     (dielectric function)
c     error           error handeling variables error = 0 success
...
c     e_br(1:max_nwave)          local complex effective permeability use for the iteration
...
c 
      subroutine bruggeman(max_nwave,max_ninc,nwave,ninc,e_mat,e_inc,
     *                           vol_frac,L1,L2,
     *                           tol,niter,e_br,iter,error,chisq) 
      implicit none
c
c --- parameters
...
c     output
      integer iter
      complex e_br(1:nwave)
c     local variables
      integer inc,wave
      double precision chisq,chisq2,mchisq,L3(1:ninc),vol_tot,min_chisq
      complex alpha1(1:max_nwave),alpha2(1:max_nwave),
     *        alpha3(1:max_nwave),alpha(1:max_nwave),
     *        sum_inc,e_br2(1:max_nwave)
...
c --- compute the polarizabilities with as first guess (e_br=e_mat)
      do wave = 1,nwave
         sum_inc = (0.0,0.0)
         do inc = 1,ninc
c           L1 + L2 + L3 = 1.0
...
c
     ...
     *   (e_mat(wave)+L3(inc)*(e_inc(inc,wave)-e_mat(wave)))
         ...
         enddo
         e_br(wave) = e_mat(wave)*(1.0d0-vol_tot+(1.0d0/3.0d0)*sum_inc)/
     *                            (1.0d0-vol_tot-(2.0d0/3.0d0)*sum_inc)
...
c --- the actual iteration starts here
      do iter=1,niter
         ...
         do wave = 1,nwave
           sum_inc = (0.0,0.0)
           do inc = 1,ninc
            alpha1(wave) = (e_inc(inc,wave)-e_br(wave))/
     *   (e_mat(wave)+L1(inc)*(e_inc(inc,wave)-e_br(wave)))
            alpha2(wave) = (e_inc(inc,wave)-e_mat(wave))/
     *   (e_mat(wave)+L2(inc)*(e_inc(inc,wave)-e_br(wave)))
            alpha3(wave) = (e_inc(inc,wave)-e_mat(wave))/
     *   (e_mat(wave)+L3(inc)*(e_inc(inc,wave)-e_br(wave)))
            alpha(wave) = (alpha1(wave)+alpha2(wave)+alpha3(wave))/3.0d0
            sum_inc = sum_inc + vol_frac(inc)*alpha(wave)
           enddo
       e_br2(wave) = e_mat(wave)*((1.0d0-vol_tot+(1.0d0/3.0d0)*sum_inc)/
     *                            (1.0d0-vol_tot-(2.0d0/3.0d0)*sum_inc))
           chisq2 = chisq2+(abs(e_br(wave)-e_br2(wave))**2.0d0)
           e_br(wave) = e_br2(wave)
        enddo
           chisq2 = chisq2 / nwave
           mchisq = abs(chisq-chisq2)/(0.5d0*(chisq+chisq2))
           chisq  = chisq2
           if (mchisq.le.tol.or.chisq.lt.1e-30) goto 1
      enddo
